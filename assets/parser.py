...
def convert_parameterized_object_set_assignment(_s, _l, tokens):
    ...
    try:
        for member_tokens in tokens[4].asList():
            # print("member_tokens:", member_tokens)
            flat = member_tokens[0]

            if len(flat) == 1 and flat[0] == '...':
                # Just skip adding members, leave as empty list
                continue

            # Look for pattern: '{', <type>, 'IDENTIFIED', 'BY', <id>, '}'
            if len(flat) == 6 and flat[0] == '{' and flat[2] == 'IDENTIFIED' and flat[3] == 'BY' and flat[5] == '}':
                member_type = flat[1]
                member_id = flat[4]
                member = {
                    'type': member_type,
            ...
            #     # fallback: capture raw if pattern unexpected
            #     members.append({'raw': flat})
        ...
        print("Error processing member tokens, skipping...")
        pass
